...
- [ ] 3. WindowRendererのワークスペース対応
- [ ] 3.1 ワークスペースのウィンドウマップ/アンマップ機能の実装
  - ウィンドウリストをマップするヘルパーメソッドを実装
  - ウィンドウリストをアンマップするヘルパーメソッドを実装
  - バッチ処理でちらつきを最小限に抑える
  - エラーが発生しても残りのウィンドウで継続
  - _Requirements: 6.1, 6.2, 6.3, 6.4_

- [ ] 3.2 apply_stateメソッドのワークスペース対応
  - 現在のワークスペースのみのウィンドウをレンダリング
  - ワークスペース切り替え時のmap/unmap処理を統合
  - 既存のジオメトリ計算ロジックを維持
...
- [ ] 5. イベント処理の修正
- [ ] 5.1 MapRequest/UnmapNotifyイベントの更新
...
- [ ] 5.2 その他のイベントハンドラの更新
  - フォーカス変更を現在のワークスペースに適用
  - fullscreen/zoom操作を現在のワークスペースに適用
  - ウィンドウ交換・回転を現在のワークスペースに適用
  - _Requirements: 4.5, 9.3_

---

- [ ] 6. ユニットテストの実装
- [ ] 6.1 Workspace構造体のユニットテスト
  - 空のワークスペース作成テスト
  - ウィンドウ操作（追加、削除、クエリ）テスト
  - フォーカス状態管理テスト
  - 状態の独立性テスト
  - _Requirements: 8.1, 8.3_

- [ ] 6.2 WindowManagerのワークスペース操作テスト
  - 単一ワークスペースでの初期化テスト
  - ワークスペース作成と切り替えテスト
  - 複数ワークスペースでの削除テスト
  - 最後のワークスペース削除のno-opテスト
  - 循環切り替え（次/前）テスト
  - intentionally_unmappedトラッキングテスト
...
- [ ] 7. 後方互換性の検証と調整
- [ ] 7.1 既存機能の動作確認
  - 既存のウィンドウ管理操作が単一ワークスペースで正常に動作することを確認
  - 既存のキーバインディングが影響を受けないことを確認
  - BSP操作（balance、rotate、swap）が正常に動作することを確認
  - _Requirements: 9.1, 9.2, 9.3, 9.4_

- [ ] 7.2 既存テストの修正
  - WindowManagerのテストをワークスペース対応に更新
  - 既存のウィンドウ操作テストが単一ワークスペースで動作することを確認
  - 必要に応じてテストのセットアップを調整
