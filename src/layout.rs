...
impl LayoutManager {
    ...
    fn tile_master_stack(
        ...
        let master_width = if num_windows > 1 {
            // Multiple windows: master takes ratio of (screen - 3*gap)
            ((screen_width - 3 * gap_i16) as f32 * master_ratio) as i16
        } else {
            // Single window: full width minus gaps on both sides
            screen_width - 2 * gap_i16
        };
        ...
        let master_config = ConfigureWindowAux::new()
            ...
            .height((screen_height - 2 * gap_i16).max(1) as u32);
        ...
        if num_windows > 1 {
            ...
            let stack_width = screen_width - stack_x - gap_i16;  // Remaining width minus right gap
            let total_stack_height = screen_height - 2 * gap_i16 - (num_stack - 1) * gap_i16;
            let stack_height = total_stack_height / num_stack;
